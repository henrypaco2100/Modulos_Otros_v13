    ...
    def _notify_thread(self, message, msg_vals=False, **kwargs):
        ...
        if author_id != partner_chatgpt.id and chatgpt_name in msg_vals.get('record_name', '') or 'ChatGPT,' in msg_vals.get('record_name', '') and self.channel_type == 'chat':
            try:
                ...
                self.with_user(user_chatgpt).message_post(body=res, message_type='comment', subtype='mail.mt_comment')
            ...
            try:
                ...
                res = response['choices'][0]['text']
                chatgpt_channel_id.with_user(user_chatgpt).message_post(body=res, message_type='comment', subtype='mail.mt_comment')
            except Exception as e:
                raise UserError(_(e))
